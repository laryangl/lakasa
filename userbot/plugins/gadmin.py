...
async def catgban(event):
    if event.fwd_from:
        return
    cate = await edit_or_reply(event, "جـآڒٍي آلـغــ۫͜ـآء حــ۫͜ـظًڒٍهہ")
    start = datetime.now()
    user, reason = await get_user_from_event(event, cate)
    if not user:
        return
...
async def startgmute(event):
    ...
    if is_muted(userid, "gmute"):
        return await edit_or_reply(
            event,
            f"هذًا اݪمـٰسـٰٖـــتخــ۫دِمـٰ مـڪتـَـــَوم ـاݪفـــعـۂݪ 𓃼≫",
        )
    try:
        mute(userid, "gmute")
...
async def endgmute(event):
    ...
    if not is_muted(userid, "gmute"):
        return await edit_or_reply(
            event, f"هہذًآ آلـمـٰســتـٰــخــ۫ـدِمـٰ غــ۫ـيڒٍ مـٰڪتـٰཻــﻭمـٰ"
        )
    try:
        unmute(userid, "gmute")
    except Exception as e:
